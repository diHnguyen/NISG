...
#                 println("\tnewP = ", newP, " find M")
                ...
                #colGen_Obj = sum(lambda[i]*sum(q_bar[a] for a in P_set[i]) for i=1:numPaths)
...
#                     println("\tM isn't empty")
                    ...
                    if colGen_Obj + sum(lambda[i] for i in lambda_pos) - pi_ < -TOL
                        println("\tMon ",M, "\t", colGen_Obj + sum(lambda[i] for i in lambda_pos) - pi_)
                    # if new_F_PM - pi_< -TOL 
